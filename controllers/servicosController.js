...
module.exports = {
    ...
    async cadastrarServicos(request,response) {
        try {
            return response.status(200).json({confirma: 'Cadastrar Servicos'});
        } catch (error) {
            return response.status(500).json({confirma: 'Erro', message: error});
        }
    },
    async editarServicos(request,response) {
        try {
            return response.status(200).json({confirma: 'Editar Servicos'});
        } catch (error) {
            return response.status(500).json({confirma: 'Erro', message: error});
        }
    },
    async excluirServicos(request,response) {
        try {
            return response.status(200).json({confirma: 'Excluir Servicos'});
        } catch (error) {
            return response.status(500).json({confirma: 'Erro', message: error});
        }
    },
